...
describe('AuthController', () => {
    ...
    describe('changeRole', () => {
        it('should change user role and send a success message when role is valid', async () => {
            authService.changeRole.mockResolvedValue(null);
            const req = { user: { id: '123' }, body: { role: 'STUDENT' } };
            const res = { send: jest.fn() } as any;
            const id = '456';

            await authController.changeRole(id, req, res);
        ...
        it('should send error if role is not valid', async () => {
            const req = { user: { id: '123' }, body: { role: 'INVALID_ROLE' } };
            const res = { send: jest.fn() } as any;
            const id = '456';
        ...
        it('should send error if role is ADMIN', async () => {
            const req = { user: { id: '123' }, body: { role: Role.ADMIN } };
            const res = { send: jest.fn() } as any;
            const id = '123';
